...
`hmm.py` loads the params from these text files (so if the param files exist, there is no need to run `src/process_mcgill_data.py`) and initialises a Guassian HMM model with those params. It then runs Viterbi for a test song and prints the optimal sequence of chords for that song. 